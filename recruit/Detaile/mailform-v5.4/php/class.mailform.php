...
class Mailform {
	...
	public function __construct() {
		...
		if ( file_exists( dirname( __FILE__ ) .'/../addon/carbon-copy/carbon-config.php' ) ) {
			...
			$this->cc_address  = $rm_cc_address;
			$this->bcc_address = $rm_bcc_address;
		}
	...
	public function mail_send() {
		...
		$send_address_split = explode( ',', $this->send_address );
		
		if ( $this->reply_mail_address === true ) {
			$additional_headers = "From: ".$this->mail_address."\r\n";
		} else {
			$additional_headers = "From: ".$send_address_split[0]."\r\n";
		}
		...
		if ( $this->reply_mail == 1 ) {
			...
			$thanks_additional_headers = "From: ".$this->send_name." <".$send_address_split[0].">";
